...
function CategoryPage() {
    ...
    const fetchCategoryInfo = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/categories/${encodeURIComponent(categoryName)}`);
            if (res.ok) {
    ...
    const handleAddToCart = async (product, quantity) => {
        ...
        if (isPreOrderRequest) {
            if (userPreOrders[product._id]) {
                ...
                try {
                    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/preorders/${existingPreOrder._id}`, {
                        method: 'PUT',
                        headers: {
